...
describe "git-switch" do
  ...
  it "lists branches order by modified date" do
    execute_command("--modified").must_equal <<-EOT.chomp
1. feature_one
2. feature_three
3. feature_two
...
EOT
  ...
  it "lists branches order by modified date using short argument" do
    execute_command("-m").must_equal <<-EOT.chomp
1. feature_one
2. feature_three
3. feature_two
Select a branch (1-3,q) 
EOT
  ...
  it "lists branches order by checked out date" do
    execute_command("--checked-out").must_equal <<-EOT.chomp
1. feature_two
2. feature_three
3. feature_one
Select a branch (1-3,q) 
EOT
  ...
  it "lists branches order by checked out date using short argument" do
    execute_command("-o").must_equal <<-EOT.chomp
1. feature_two
2. feature_three
3. feature_one
Select a branch (1-3,q) 
EOT
  ...
  it "limits the number of branches to show" do
    execute_command("--count 2").must_equal <<-EOT.chomp
1. feature_one
2. feature_three
...
EOT
  ...
  it "limits the number of branches to show using short argument" do
    execute_command("-c 2").must_equal <<-EOT.chomp
1. feature_one
2. feature_three
Select a branch (1-2,q) 
EOT
